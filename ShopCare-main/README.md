...
| 测试集 Micro-F1 | **0.672** | **0.741** | 未训练(见下) |
| 测试集 Macro-F1 | 0.679 | 0.762 | - |
| 平均置信度 | 0.598 | 0.717 | - |
| 自动分流率 / 拒识率 | 77.6% / 22.4% | 82.1% / 17.9% | - |
| 自动分流 Micro-F1 | 0.701 | 0.785 | - |
| 标定阈值(单标签/全局) | 0.25 / 0.443 | 0.30 / 0.506 | - |
| 概率校准 ECE(校准前→后) | 0.081 → **0.027** | 0.062 → **0.022** | - |
...
> **拒识率 22.4% / 17.9% 高于 15% 的业务目标**, 这是**标定集(dev)与评测集(test)的难度差**
> 造成的, 不是模型或代码问题: 两个 split 各只有 108 个模板家族, 随机一分难度就会有差异,
> 实测 dev 上有 96.6% 的工单能激活标签, test 上只有 91.3%。在 dev 上把拒识率标到 13.5%,
> 换到 test 就漂到 22.4%。真实上线应当用线上数据持续重新标定, 而不是一次标定用到底。
...
| | 校准前 | 校准后 | 校准后实际含义 |
|---|---|---|---|
| `02-rf` | invoice = 0.419 | invoice = **0.697** | "大约 70% 的把握是发票问题" |
| `03-fasttext` | invoice = **1.000** | invoice = **0.855** | "大约 86% 的把握是发票问题" |
...
| | ECE(越低越准) | 平均置信度 | 预测标签数 vs 真实 |
|---|---|---|---|
| RF 校准前 | 0.081 | 0.422 | 1.50 vs 1.55 |
| RF 校准后 | **0.027** | 0.598 | 1.77 vs 1.55 |
| FastText 校准前 | 0.062 | 0.779 | 1.33 vs 1.55 |
| FastText 校准后 | **0.022** | 0.717 | 1.53 vs 1.55 |
...
> 同一把尺子上, 所以阈值可以直接互相比较(RF 0.25/0.443 vs FastText 0.30/0.506);
...
1. **这是合成语料上的数字**, 且 train/test 的模板家族完全不重叠 —— 也就是说 test 考的是
   ...
   比这个高。
2. **业务上真正该看的是"自动分流 Micro-F1 / 分流率"这一对数字**: FastText 在 82.1% 的
   工单上自动分流, 这部分准确度 0.785; 剩下 17.9% 拒识转人工/LLM。对客服场景来说,
