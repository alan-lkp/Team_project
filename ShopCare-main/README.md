...
| 体积 | 61.4 MB | 19.6 MB | 视 LoRA 配置, 通常 10~60 MB |
| 训练耗时 | 75 s | 4 s | 分钟级(CPU 更久) |
| 测试集 Micro-F1 | **0.670** | **0.749** | 未训练(见下) |
| 测试集 Macro-F1 | 0.667 | 0.769 | - |
| 平均置信度 | 0.621 | 0.687 | - |
| 自动分流率 / 拒识率 | 81.2% / 18.8% | 82.2% / 17.8% | - |
| 自动分流 Micro-F1 | 0.719 | 0.790 | - |
| 标定阈值(单标签/全局) | 0.40 / 0.455 | 0.35 / 0.476 | - |
| 概率校准 ECE(校准前→后) | 0.123 → **0.014** | 0.057 → **0.016** | - |
| 语料外新说法 | 容易漏判/拒识 | 较好 | 最好 |

> **"平均置信度"是可以跨模型比较的**, 因为两个模型都做了概率校准(见下文第五节)。
> 校准前 RF 只会说 0.42、FastText 只会说 1.000 —— 同一件事, 一个像是模型坏了、
> 一个像是数据泄漏, 根本没法放在一张表里。

> **拒识率 18.8% / 17.8% 略高于 15% 的业务目标**, 这是**标定集(dev)与评测集(test)的难度差**
> 造成的, 不是模型或代码问题: 两个 split 各只有 108 个模板家族, 随机一分难度就会有差异,
> 实测 dev 上有 96.6% 的工单能激活标签, test 上只有 91.3%。在 dev 上把拒识率标到 ~15%,
> 换到 test 就会漂到 18% 上下。真实上线应当用线上数据持续重新标定, 而不是一次标定用到底。
...
两个模型的**原始输出尺度完全不同**,都不能当"可信度"直接读。同一条工单
「发票开错了, 抬头写成公司旧名字, 能重开吗」:

| | 校准前 | 校准后 | 校准后实际含义 |
|---|---|---|---|
| `02-rf` | invoice = 0.538 | invoice = **0.912** | "大约 91% 的把握是发票问题" |
| `03-fasttext` | invoice = **1.000** | invoice = **0.890** | "大约 89% 的把握是发票问题" |

两个模型**标签都判对了**, 但校准前一个显示 42%(像是模型坏了)、一个显示 100%
(像是数据泄漏)。原因:

- RF 的概率是 300 棵树在叶子上的投票比例, 天然被压向中间 —— 实测它**偏保守**
  (它说 0.6~0.8 时, 实际命中率是 100%, 但它永远不会给出 1.0);
- FastText 用 ova(每标签一个独立 sigmoid), 训练 25 轮后会**饱和** ——
  5.9% 的标签概率正好是 1.000, 而那一档的实际命中率只有 88%。

**做法**: `tools/prob_calibration.py` 在 **dev** 上为每个标签单独拟合一个单调映射
(Platt scaling), 训练与推理两端都过一遍。单调保证**不改变排序**, 所以 Micro-F1
不是靠校准"调"上去的; 变的是"概率"这个数字本身的含义。test 不参与拟合, 指标依然干净。

校准带来的收益(在 test 上实测):

| | ECE 校准前 | ECE 校准后 | 平均置信度 | 预测标签数 vs 真实 |
|---|---|---|---|---|
| `02-rf` | 0.123 | **0.014** | 0.621 | 1.29 vs 1.47 |
| `03-fasttext` | 0.057 | **0.016** | 0.687 | 1.48 vs 1.47 |

> **为什么默认用 Platt 而不是保序回归(isotonic)**: 两者都试过 —— isotonic 在 dev 上的
> 对数损失更低, 但那是**拟合过度**: dev 里恰好 100% 命中的分箱会被直接映射成 1.000,
> 换到 test 上就成了新的过度自信(2.3% 的标签输出正好 1.0, 而 Platt 只有 0.3%)。
> 而"显示 100%"正是这套系统想避免的东西。实测 test ECE 也是 Platt 更好
> (0.027 vs 0.035)。

> **阈值为什么不是 0.5/0.8**: 两个模型都用 `--tune-threshold` 在 dev 上按业务目标
> (拒识率 ≤ 15% 前提下最大化自动分流 Micro-F1) 标定。现在两个模型的概率已经校准到
> 同一把尺子上, 所以阈值可以直接互相比较(RF 0.40/0.455 vs FastText 0.35/0.476);
> 差别来自两者"敢不敢下判断"的性格不同, 不再是尺度不可比。
...
| 输入 | 修复前 | 修复后 |
|---|---|---|
| `物流` | **invalid 0.95** | logistics(见下注) |
| `快递一直没到` (6 字) | **invalid 1.00** | **logistics 0.85** |
| `包裹一直没到` | 全标签 ≈ 0 | **logistics 0.85** |
| `东西还没发货` | 全标签 ≈ 0 | **logistics 0.85** |
| `我的快递一直没到` (8 字) | logistics 1.00 ✅ | logistics 1.00 ✅ |
...
上文"三套对照模型"的说明。**现在 (RF 0.599 / FastText 0.750) 的数字没有这个问题**:
语料重建后 test 子句在 train 中原样出现的比例是 0.0000%。

**Q: 那 Micro-F1 只有 0.67~0.74, 是不是说明模型不行?**
不完全是。三点要看清楚:

1. **这是合成语料上的数字**, 且 train/test 的模板家族完全不重叠 —— 也就是说 test 考的是
   "没见过的说法"。真实业务语料里同一个诉求会反复以相似措辞出现, 同分布下的指标通常会
   比这个高。
2. **业务上真正该看的是"自动分流 Micro-F1 / 分流率"这一对数字**: FastText 在 82.2% 的
   工单上自动分流, 这部分准确度 0.790; 剩下 17.8% 拒识转人工/LLM。对客服场景来说,
   这个组合比一个好看的全局 F1 更有意义。
3. **两个模型的差距方向是对的**: 纯字面 n-gram 的 RF(0.672)明显弱于带词向量与
   n-gram 语义的 FastText(0.741)。这正是这张表存在的意义 —— 回答
   "我们真的需要上更重的模型吗"。
...
- **拒识率在 dev 上标定、在 test 上会漂**: 目标 15%, dev 上标到 13.5%, test 实测 22.4%。
  根因是 dev / test 各只有 108 个模板家族, 随机切分带来的难度差就足以让激活率从 96.6%
  掉到 91.3%。真实上线必须用线上数据**持续**重标定, 不能一次标定用到底。
