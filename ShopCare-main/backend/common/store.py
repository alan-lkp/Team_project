...
class MySQLStore:
    ...
    def _new_conn(self):
        ...
        return pymysql.connect(
            host=self.cfg.db_host, port=self.cfg.db_port, user=self.cfg.db_user,
            password=self.cfg.db_password, database=self.cfg.db_name,
            charset=self.cfg.db_charset, autocommit=False,
            connect_timeout=self.cfg.db_connect_timeout, cursorclass=pymysql.cursors.DictCursor,
        )
    ...
    def _execute_plain(self, sql):
        ...
        import pymysql
        conn = pymysql.connect(host=self.cfg.db_host, port=self.cfg.db_port,
                               user=self.cfg.db_user, password=self.cfg.db_password,
                               charset=self.cfg.db_charset, autocommit=True,
...
class RedisStore:
    ...
    def connect(self):
        ...
        try:
            import redis
            client = redis.Redis(host=self.cfg.redis_host, port=self.cfg.redis_port,protocol=2,
...
class Store:
    ...
    def _ensure_admin_mysql(self):
        rows = self.mysql.query('SELECT id FROM users WHERE username=%s', (self.cfg.admin_username,))
        if rows:
            return
        self.mysql.execute(
    ...
    def _ensure_admin_memory(self):
        existing = self.memory.select('users', where={'username': self.cfg.admin_username})
        if existing:
            return
        self.memory.insert('users', {'username': self.cfg.admin_username,
                                     'password_hash': hash_password(self.cfg.admin_password),
                                     'role': 'admin'})

    def health(self):
        return {
            'mysql': {'available': self.mysql.available(), 'error': self.mysql.last_error,
                      'host': self.cfg.db_host, 'database': self.cfg.db_name},
            'redis': {'available': self.redis.available(), 'error': self.redis.last_error,
    ...
    def get_user(self, username):
        if self.mysql.available():
            return self.mysql.query('SELECT * FROM users WHERE username=%s', (username,), one=True)
    ...
    def create_user(self, username, password, role='user'):
        ...
        if self.mysql.available():
            ...
            return rid, None
        rid = self.memory.insert('users', {'username': username,
                                           'password_hash': hash_password(password), 'role': role})
        return rid, None

    # ---------------- 工单 ----------------
    TICKET_FIELDS = ('ticket_id', 'text', 'text_masked', 'model_used', 'labels', 'confidences',
                     'avg_confidence', 'rejected', 'reject_reason', 'sentiment', 'priority',
                     'dept', 'sla_hours', 'suggested_reply', 'needs_approval', 'resolved_by',
    ...
    def get_ticket(self, ticket_id):
        if self.mysql.available():
            row = self.mysql.query('SELECT * FROM tickets WHERE ticket_id=%s', (ticket_id,), one=True)
        else:
            rows = self.memory.select('tickets', where={'ticket_id': ticket_id}, limit=1)
            row = rows[0] if rows else None
        return self._decode_ticket(row)

    def list_tickets(self, page=1, size=20, status=None, priority=None, dept=None,
                     model_used=None, rejected=None, keyword=None):
        ...
        if self.mysql.available():
            where, args = [], []
            for col, val in (('status', status), ('priority', priority), ('dept', dept),
            ...
            total_row = self.mysql.query(f'SELECT COUNT(*) AS n FROM tickets {clause}', tuple(args), one=True)
    ...
    def update_ticket(self, ticket_id, fields):
        if self.mysql.available():
            ...
            if not cols:
                return 0
            sql = 'UPDATE tickets SET ' + ','.join(f'{c}=%s' for c in cols) + ' WHERE ticket_id=%s'
            rowcount, _ = self.mysql.execute(sql, tuple(fields[c] for c in cols) + (ticket_id,))
            return rowcount
        rows = self.memory.select('tickets', where={'ticket_id': ticket_id}, limit=1)
        return self.memory.update('tickets', rows[0]['id'], fields) if rows else 0

    # ---------------- 人工复核 ----------------
    def create_review(self, ticket_id, action, corrected_labels=None, note=None, reviewer=None):
        rec = {
            'ticket_id': ticket_id,
        ...
        if self.mysql.available():
            _, rid = self.mysql.execute(
                ...
                'VALUES(%s,%s,%s,%s,%s)',
                (rec['ticket_id'], rec['action'], rec['corrected_labels'], rec['note'], rec['reviewer']))
        else:
    ...
    def list_reviews(self, page=1, size=20):
        ...
        offset = (page - 1) * size
        if self.mysql.available():
            total_row = self.mysql.query('SELECT COUNT(*) AS n FROM reviews', (), one=True)
            total = int(total_row['n']) if total_row else 0
            rows = self.mysql.query('SELECT * FROM reviews ORDER BY id DESC LIMIT %s OFFSET %s',
                                    (size, offset))
        else:
            total = self.memory.count('reviews')
            rows = self.memory.select('reviews', order_by='id', desc=True, limit=size, offset=offset)
        return {'items': rows, 'total': total, 'page': page, 'size': size,
                'pages': (total + size - 1) // size if size else 0}

    # ---------------- 审计 ----------------
    def log_action(self, username, action, target=None, detail=None):
        rec = {'username': username, 'action': action, 'target': target,
    ...
    def list_audit(self, page=1, size=50):
        ...
        if self.mysql.available():
            rows = self.mysql.query('SELECT * FROM audit_logs ORDER BY id DESC LIMIT %s OFFSET %s',
                                    (size, offset))
        else:
            rows = self.memory.select('audit_logs', order_by='id', desc=True, limit=size, offset=offset)
        return {'items': rows, 'size': size, 'page': page}

    # ---------------- 统计 ----------------
    def _all_tickets(self):
        if self.mysql.available():
            return [self._decode_ticket(r) for r in self.mysql.query('SELECT * FROM tickets ORDER BY id DESC')]
        return [self._decode_ticket(r) for r in self.memory.select('tickets', order_by='id', desc=True)]
    ...
    def stats_summary(self):
        ...
        total = len(tickets)
        if total == 0:
            return {'total': 0, 'auto_rate': 0.0, 'reject_rate': 0.0, 'avg_latency_ms': 0.0,
                    'priority_dist': {}, 'dept_dist': {}, 'model_dist': {}, 'sentiment_dist': {},
                    'trend': [{'date': d, 'count': 0} for d in day_list(7)], 'top_labels': [],
                    'pending_review': 0}
        ...
        return {
            ...
            'avg_confidence': round(sum(t.get('avg_confidence') or 0 for t in tickets) / total, 4),
            ...
            'top_labels': [{'label': k, 'count': v} for k, v in
                           sorted(label_counter.items(), key=lambda kv: -kv[1])[:9]],
...
if __name__ == '__main__':
    ...
    st.memory.insert('tickets', {'ticket_id': 'SC-TEST-1', 'text': '快递一直没到',
                                 'labels': json.dumps([{'label': 'logistics', 'score': 0.9}]),
                                 'confidences': '{}', 'rejected': False, 'priority': 'P1',
                                 'dept': '物流仓储部', 'model_used': 'rf', 'sentiment': 'negative',
                                 'avg_confidence': 0.9, 'latency_ms': 12.3, 'status': 'pending'})
    ...
    print('统计:', {k: summary[k] for k in ('total', 'auto_rate', 'reject_rate', 'trend')})
