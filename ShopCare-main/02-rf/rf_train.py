...
def train_ovr(X, Y, cfg, verbose=True):
    ...
    for i, name in enumerate(cfg.class_list):
        ...
        if n_pos == 0 or n_pos == n_all:
            ...
            constants.append({'type': 'degenerate', 'prob': round(prior, 4),
                              'note': '训练集中该标签只有单一类别'})
        ...
        if verbose:
            print(f'    [{i + 1}/{cfg.num_labels}] {name:<16} 正例 {n_pos:>5} 条  '
                  f'({n_pos / n_all:.1%})  已训练')
...
def top_features(vectorizer, estimators, cfg, top_n=15):
    ...
    for i, name in enumerate(cfg.class_list):
        ...
        out[name] = [{'feature': str(names[j]), 'importance': round(float(imp[j]), 6)}
                     for j in idx if imp[j] > 0]
    idx = np.argsort(global_imp)[::-1][:top_n]
    out['_global'] = [{'feature': str(names[j]), 'importance': round(float(global_imp[j]), 6)}
                      for j in idx if global_imp[j] > 0]
    return out
...
def run(cfg, args):
    ...
    print('  最长尾的三个标签:', ', '.join(f'{r["label"]}({r["count"]})' for r in long_tail))
    ...
    # ---------- 3. 训练 ----------
    params_list = cfg.grid_search_space if (args.grid_search or cfg.enable_grid_search) else [None]
    best = None
    for k, params in enumerate(params_list, 1):
        ...
        if best is None or m['micro_f1'] > best['metrics']['micro_f1']:
            best = {'metrics': m, 'estimators': estimators, 'constants': constants, 'params': params}
    ...
    if args.tune_threshold:
        ...
        if grid:
            ...
            print('  Top-5:')
            for r in grid[:5]:
                print(f'    label={r["label_threshold"]} global={r["global_threshold"]} '
    ...
    bundle = {
        ...
        'params': {'n_estimators': cfg.n_estimators, 'max_depth': cfg.max_depth,
                   'min_samples_leaf': cfg.min_samples_leaf, 'rf_max_features': cfg.rf_max_features,
                   'class_weight': cfg.class_weight, 'ngram': cfg.word_ngram_range if mode == 'word' else cfg.char_ngram_range},
...
def main():
    ...
    parser.add_argument('--grid-search', action='store_true', help='额外做一轮小规模超参搜索')
