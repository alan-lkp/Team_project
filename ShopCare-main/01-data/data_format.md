...
现在改为 `tools/build_dataset_v2.py`，四层修复：

1. **扩池**：模板从 123 条扩到 540 条长句（每个标签 60 条，见 `tools/_tpl_part{1,2,3}.py`）
   + 269 条短句（每个标签 30 条，3~12 字，见 `tools/_tpl_short{1,2,3}.py`）；
...
4. **短句覆盖**：15% 的样本是单句短文本。原来的语料里只有 `invalid` 含超短样本，
   模型因此学出「短文本 → 无效与恶意」的长度先验（`快递一直没到` 会被判成无效），
   给每个标签都补上短句后这个问题消失。详见 README「4.2 长度先验」。
